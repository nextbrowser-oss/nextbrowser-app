...
export function AutomationStudio() {
  ...
  const workspaceId = s.activeWorkspaceId || "default";
  ...
  useEffect(() => {
    if (!selectedWorkflowId && s.localSkills[0]) setSelectedWorkflowId(s.localSkills[0].id);
  }, [s.localSkills, selectedWorkflowId]);

  useEffect(() => {
    const selected = s.localSkills.find((skill) => skill.id === selectedWorkflowId);
    setDraft(selected ? structuredClone(selected) : undefined);
    setActionErrors({});
  }, [selectedWorkflowId, s.localSkills]);
  ...
  const saveCapture = async (run: CapturedRun) => {
    await s.saveLocalSkill(skillFromRun(run));
  ...
  const saveDraft = async () => {
    ...
    setSaving(true);
    try { await s.saveLocalSkill({ ...draft, recipe: { ...draft.recipe, actions: draft.actions } }); }
  ...
  const createWorkflow = async () => {
    ...
    await s.saveLocalSkill(workflow);
  ...
  return (
    <div className="automation-studio page">
      <header className="automation-hero">
        ...
        <div className="automation-summary"><strong>{s.localSkills.length}</strong><span>workflows</span><strong>{artifacts.length}</strong><span>artifacts</span></div>
      </header>
      ...
      {section === "recorder" && <section className="automation-panel">
        <div className="automation-panel-head"><div><h2>Browser Automation Recorder</h2><p>Capture semantic browser tool calls from the next agent run.</p></div>
          <button className="primary" onClick={startRecording}><Icon name="circle.fill" size={12} /> {recordingSince ? "Record another run" : "Record next run"}</button>
        </div>
