...
async function listAutomationShares(box, deps) {
  const body = await projectRequest(`/v1/automation/shares?box=${encodeURIComponent(box || "inbox")}`, {}, deps);
  return body?.shares || [];
}
...
const updateAutomationRunStep = (id, position, update, deps) => projectRequest(`/v1/automation/runs/${encodeURIComponent(id)}/steps/${encodeURIComponent(position)}`, { method: "PUT", body: JSON.stringify(update) }, deps);
