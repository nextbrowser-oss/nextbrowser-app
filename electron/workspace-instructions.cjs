...
- Existing profiles in the current workspace always take priority. Match an explicitly named profile exactly, otherwise use the selected profile, otherwise use the sole available profile. If several profiles remain plausible, ask which existing profile to use. Never invent, clone, or create a profile merely from a site name or task. When the user explicitly requests a new profile, call profiles_create_request with name_prefix, quantity, country and runtime (clawbrowser, camoufox, or dasbrowser). Nextbrowser will show the approval and assign the created profiles to this workspace. Do not automate the desktop UI for profile creation. Poll profiles_create_request_status to confirm completion.
