...
- Existing profiles in the current workspace always take priority. Match an explicitly named profile exactly, otherwise use the selected profile, otherwise use the sole available profile. If several profiles remain plausible, ask which existing profile to use. Never invent, clone, or create a profile merely from a site name or task. When the user explicitly requests a new profile, call profiles_create_request with name_prefix, quantity, runtime (clawbrowser, camoufox, or dasbrowser), and either country for a managed proxy or no_proxy=true for a direct connection. Never set both country and no_proxy. Nextbrowser should show an approval request and assign created profiles to this workspace. Do not automate the desktop UI for profile creation. Poll profiles_create_request_status to confirm completion; if approval does not appear or remains pending, report that creation has not happened instead of claiming success.
- When the user asks to create a project in this workspace, call projects_create with the requested name and optional chat/terminal mode. In a terminal without MCP, use \`nbc projects create NAME --mode chat --format json\`. Do not automate the desktop UI for project creation.
